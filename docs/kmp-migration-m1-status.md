...
- **AGP 9 / Gradle 9 upgrade** — tracked in [#16](https://github.com/appswithlove/updraft-sdk-android/issues/16) (decided 2026-07-22: do it, but as its own milestone — not in M2, which was device-verified on AGP 8.13): **implemented on branch `feature/toolchain-agp9`** with the final version set — Gradle 9.6.1, AGP 9.3.1, Kotlin 2.2.21 (unchanged), Compose Multiplatform 1.10.3 (not 1.11.1 — 1.11.x requires Kotlin >2.2.21, deferred), vanniktech maven-publish 0.37.0, dokka 2.2.0, loco 1.2.0 (`locoFetch`/`locoPush`), androidx.startup 1.2.0, coveralls 2.12.2 (kept). Built-in Kotlin migration done for `:updraft-sdk`; KMP modules migrated to the `com.android.kotlin.multiplatform.library` target plugin. `sample` split into a shared KMP library module + `androidApp` (AGP 9 forbids a single module being both an Android app and a KMP target). The updraft gradle plugin was bumped to 3.0.0 (the AGP 9-compatible line; 2.x crashes under AGP 9). Android host tests now run via `testAndroidHostTest` (`withHostTestBuilder`). Requires a re-verification round on device afterwards.
